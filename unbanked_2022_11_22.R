...
setwd("~/Papers/unbanked/unbanked_coding")
dir()

### Reading RDS datasets
i1.df = readRDS("dcpc_2021_indlevel_public.rds")# indiv data
t1.df = readRDS("dcpc_2021_tranlevel_public.rds")# trans data
d1.df = readRDS("dcpc_2021_daylevel_public.rds" )# day data
...
### Table 2 (variant): banked/unbanked adoption of checking and savings accounts
...
# building a data frame for Table 2 (all weighted)
...
#
write.csv(own.df, "table2.csv", row.names = F)

### Start Figure 1 (in 2016 paper)
...
#
write.csv(char.df, "figure1.csv", row.names = F)

### Start Figure 2 (in 2016 paper)
nrow(i5.df)
sum(i5.df$w5)
#
banked_num
...
# finalizing Figure 2 in 2016 paper
...
#
write.csv(income.df, "figure2.csv", row.names = F)

### Start Table A.1: Descriptive stats (banking by demographics)
summary(i5.df$age)
...
(demog_income_var.vec = c("Household income: 0-30k", "Household income: 30-60k", "Household income: 60-90k", "Household income: 90 and higher"))
#
...
# construct a data frame to be appended to demog
(demog_resp_var.vec = c("Respondents: Number", "Respondents: % of all respondents"))
#
...
## combine all demog data frames into one (equivalent of Table A.1 in the 2016 paper)
#
demog_resp.df
dim(demog_resp.df)
#
...
write.csv(demog.df, "tableA1.csv", row.names = F)

### Start Table B.1: Demog regressions
...
# Changing education factor to "college and higher only"
str(i6.df$highest_education)
table(i6.df$highest_education, exclude = T)
i6.df$edu_factor = NA #construct age factors
#i6.df$edu_factor[i6.df$highest_education < 9] = "Education_less_than_high_school"
#i6.df$edu_factor[i6.df$highest_education == 9] = "Education_high_school"
#i6.df$edu_factor[i6.df$highest_education %in% c(10, 11, 12)] = "Education_some_college_or_associate"
#i6.df$edu_factor[i6.df$highest_education == 13] = "Education_college"
i6.df$edu_factor[i6.df$highest_education >= 13] = "Education_college_and_higher"
#
...
# start logit regressions (both types of marginal effects)
#
# model 1 has all variables as in the random forest below. Problem is with education (correlated with income). 
unbanked_reg_model1 = unbanked ~ age + gender_factor + race_factor + ethnicity_factor +  edu_factor + marital_factor + income_hh_div_10k + work_factor + hh_size + homeowner_factor
#
(unbanked_ame.reg1 = logitmfx(unbanked_reg_model1, data = i6_reg.df, atmean = F))# => cannot get education right regardless of how I 

# model 2, remove education (becomes the benchmark model)
unbanked_reg_model2 = unbanked ~ age + gender_factor + race_factor + ethnicity_factor + marital_factor + income_hh_div_10k + work_factor + hh_size + homeowner_factor
#
(unbanked_ame.reg2 = logitmfx(unbanked_reg_model2, data = i6_reg.df, atmean = F))#  
#
# model 3, remove education, hh_size, and homeowner
unbanked_reg_model3 = unbanked ~ age + gender_factor + race_factor + ethnicity_factor + marital_factor + income_hh_div_10k + work_factor
#
(unbanked_ame.reg3 = logitmfx(unbanked_reg_model3, data = i6_reg.df, atmean = F))# => cannot get education right regardless of how I 
#
# model 4, remove education, hh_size, homeowner, gender, and ethnicity
unbanked_reg_model4 = unbanked ~ age + race_factor + marital_factor + income_hh_div_10k + work_factor
#
(unbanked_ame.reg4 = logitmfx(unbanked_reg_model4, data = i6_reg.df, atmean = F))# => cannot get education right regardless of how I 
#

# Just try regression models 2,3,4 with atmean=T (marginal effect at mean) => less significant results, do not use
#(unbanked_mem.reg2 = logitmfx(unbanked_reg_model2, data = i6_reg.df, atmean = T))#  
#
#(unbanked_mem.reg3 = logitmfx(unbanked_reg_model3, data = i6_reg.df, atmean = T))#  
#
#(unbanked_mem.reg4 = logitmfx(unbanked_reg_model4, data = i6_reg.df, atmean = T))#  


# Saving Table B.1 in 2016 paper
# converting to HTML (can be read by Excel)
htmlreg(list(unbanked_ame.reg2, unbanked_ame.reg3,unbanked_ame.reg4), custom.model.names = c("Model 1", "Model 2", "Model 3"), digits = 4, single.row = T)

## Random forest VIP of the above full regression
#
# Below a model with Age factor (switched to continuous)
#unbanked_rf_model1 = unbanked ~ age_factor + gender_factor + race_factor + ethnicity_factor +  edu_factor + marital_factor + income_hh_div_10k + work_factor + hh_size + homeowner_factor
summary(i6.df$age)
# also, remove education
unbanked_rf_model2 = unbanked ~ age + gender_factor + race_factor + ethnicity_factor + marital_factor + income_hh_div_10k + work_factor + hh_size + homeowner_factor

# Remove HH size
#unbanked_rf_model3 = unbanked ~ age + gender_factor + race_factor + ethnicity_factor +  edu_factor + marital_factor + income_hh_div_10k + work_factor + homeowner_factor
#
set.seed(1955)
unbanked.rf=randomForest(unbanked_rf_model2, data=i6_reg.df, mtry=3, importance=T, na.action=na.roughfix)
#
...
## Classification tree using UpSampling using the above regression model
tree_data.df = i6_reg.df
dim(tree_data.df)
table(tree_data.df$unbanked, exclude = F)
#
# Modification of variables and variable names for the tree
colnames(tree_data.df)
str(tree_data.df)
#
tree_data.df$HH_income = tree_data.df$income_hh_div_10k * 10000 # new var expressed in dollar unit
summary(tree_data.df$HH_income)
#
colnames(tree_data.df)[colnames(tree_data.df)== "age"] = "Age"

colnames(tree_data.df)[colnames(tree_data.df)== "gender_factor"] = "Gender"
table(tree_data.df$Gender)
tree_data.df$Gender = as.factor(ifelse(tree_data.df$Gender == "Gender_male", "Male", "Female"))
#
colnames(tree_data.df)[colnames(tree_data.df)== "race_factor"] = "Race"
table(tree_data.df$Race)
tree_data.df$Race = as.factor(ifelse(tree_data.df$Race == "Race_white", "White", ifelse(tree_data.df$Race == "Race_asian", "Asian", ifelse(tree_data.df$Race == "Race_black", "Black", "Other")  )))
#
colnames(tree_data.df)[colnames(tree_data.df)== "marital_factor"] = "Married"
table(tree_data.df$Married)
#
colnames(tree_data.df)[colnames(tree_data.df)== "work_factor"] = "Work"
table(tree_data.df$Work)
tree_data.df$Work = as.factor(ifelse(tree_data.df$Work == "Work_not_employed", "not_employed", "employed"))
#
colnames(tree_data.df)[colnames(tree_data.df)== "homeowner_factor"] = "Homeowner"
table(tree_data.df$Homeowner)
tree_data.df$Homeowner = as.factor(ifelse(tree_data.df$Homeowner == "Homeowner_yes", "Yes", "No"))
#
colnames(tree_data.df)[colnames(tree_data.df)== "ethnicity_factor"] = "Ethnicity"
#
colnames(tree_data.df)[colnames(tree_data.df)== "hh_size"] = "HH_size"
#
# Remove education
colnames(tree_data.df)
tree_data.df = subset(tree_data.df, select = -c(edu_factor))
#
table(tree_data.df$unbanked)

# # Tree model (using SMOT): Removing education factor from the model and using new variable names to be displayed 
# unbanked_tree_smote_model = unbanked ~ Age + Gender + Race + Ethnicity + Married + HH_income + Work + HH_size + Homeowner
# 
# # Start balancing the data either via SMOTE or UpSampling (caret)
# set.seed(1955)
# tree_data_smote.df = smote(unbanked_tree_smote_model, tree_data.df, perc.over = 1, perc.under = 2)# => SMOTE not used
# table(tree_data_smote.df$unbanked)
# 
# #i6_reg_balanced.df = smote(unbanked_rf_model2, i6_reg.df) # using default=> SMOTE not used, use ROSE instead
# # ROSE below generates negative income
# #i6_reg_balanced.df = ROSE(unbanked_tree_model, data=i6_reg.df)$data => switch to upsampling
# #i6_reg_balanced.df = upSample(x=i6_reg.df[,-which(colnames(i6_reg.df)=="unbanked")], y=i6_reg.df$unbanked)# => create a variable called "Class" instead of "unbanked"
# #
# nrow(tree_data_smote.df)# total num resp in the synthetic balanced data
# length(unique(tree_data_smote.df$id))# so id have been duplicated
# set.seed(1955)# to be able to reproduce the rpart CV below
# unbanked_tree_smote = rpart(unbanked_tree_smote_model, data = tree_data_smote.df, method = "class", control = rpart.control(cp = 0.001))# Extremely-long tree first, then prune it
# #Below, plot a tree (Note: Longer than optimal, but needed for later prunning and redrawing). 
# prp(unbanked_tree_smote, type = 3, box.palette = "auto", extra = 100, under = T, tweak = 1.0, varlen = 0, faclen = 0)#faclet=0 avoids abvreviations, tweak for char size
# #now search for optimal cp, rpart has cp table built in
# plotcp(unbanked_tree_smote)# plot cp: Not used for this demo plot. See training data below
# names(unbanked_tree_smote)
# unbanked_tree_smote$cptable # List cp, number of splits and errors
# # Below, I choose cp to use for prunning (highest rel error below the dashed line)
# (cp.choice = unbanked_tree_smote$cptable[7, "CP"]) # Corresponds to 9 splits (just for demonstration)
# unbanked_smote_prune = prune.rpart(unbanked_tree_smote, cp=cp.choice)
...
# prp(unbanked_smote_prune, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA,  under = T, tweak = 1.1, varlen = 0, faclen = 0, Margin = 0.0, digits = -2)#faclet=0 avoids abbreviations, tweak for char size. extra = 100 omitted
# # End SMOTE tree [not used]


# Tree model (UpSampling instead of SMOTE): Removing education factor from the model and using new variable names to be displayed

tree_data_upsampling.df = upSample(x=tree_data.df[,-which(colnames(tree_data.df)=="unbanked")], y=tree_data.df$unbanked)# => create a variable called "Class" instead of "unbanked"
#
colnames(tree_data_upsampling.df)
table(tree_data_upsampling.df$Class)
tree_data_upsampling.df$Class = ifelse(tree_data_upsampling.df$Class == 1, "Unbanked", "Banked") #
#
unbanked_tree_upsampling_model = Class ~ Age + Gender + Race + Ethnicity + Married + HH_income + Work + HH_size + Homeowner
# run rpart
set.seed(1955)# to be able to reproduce the rpart CV below
unbanked_tree_upsampling = rpart(unbanked_tree_upsampling_model, data = tree_data_upsampling.df, method = "class", control = rpart.control(cp = 0.001))# Extremely-long tree first, then prune it
#Below, plot a tree (Note: Longer than optimal, but needed for later prunning and redrawing). 
...
#now search for optimal cp, rpart has cp table built in
plotcp(unbanked_tree_upsampling)# plot cp: Not used for this demo plot. See training data below
names(unbanked_tree_upsampling)
unbanked_tree_upsampling$cptable # List cp, number of splits and errors
# Below, I choose cp to use for prunning (highest rel error below the dashed line)
(cp.choice = unbanked_tree_upsampling$cptable[7, "CP"]) # Corresponds to 9 splits (just for demonstration)
unbanked_upsampling_prune = prune.rpart(unbanked_tree_upsampling, cp=cp.choice)
#prp(unbanked_prune1, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA, extra = 100, under = T, tweak = 1.1, varlen = 0, faclen = 0, Margin = 0.0, digits = -2)#faclet=0 avoids abbreviations, tweak for char size
#Below, I remove extra = 100 to remove percentagof observations. This is because the upsampled data has different number of obs.
prp(unbanked_upsampling_prune, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA,  under = T, tweak = 1.1, varlen = 0, faclen = 0, Margin = 0.0, digits = -2)#faclet=0 avoids abbreviations, tweak for char size. extra = 100 omitted

### Start Figure 3 in 2016 paper (7 reasons for not having a checking account)
dim(i5_unbanked.df)
...
# constructing data frame for fig 3
...
### Start Figure 5 in the 2016 paper (assessments)
## cost assessment table
table(i5_banked.df$as003_a2, exclude = F)
str(i5.df$as003_a2)
# banked cost begin
(banked_cost_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a2, w5_banked))), weighted.mean(as003_a2, w5_banked)))
#
table(i5_banked.df$as003_b2, exclude = F)
(banked_cost_check = with(na.omit(subset(i5_banked.df, select = c(as003_b2, w5_banked))), weighted.mean(as003_b2, w5_banked)))
#
table(i5_banked.df$as003_c2, exclude = F)
(banked_cost_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c2, w5_banked))), weighted.mean(as003_c2, w5_banked)))
#
table(i5_banked.df$as003_d2, exclude = F)
(banked_cost_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d2, w5_banked))), weighted.mean(as003_d2, w5_banked)))
#
table(i5_banked.df$as003_e2, exclude = F)
(banked_cost_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e2, w5_banked))), weighted.mean(as003_e2, w5_banked)))
#
table(i5_banked.df$as003_h2, exclude = F)
(banked_cost_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h2, w5_banked))), weighted.mean(as003_h2, w5_banked)))
#
table(i5_banked.df$as003_i2, exclude = F)
(banked_cost_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i2, w5_banked))), weighted.mean(as003_i2, w5_banked)))
#
# unbanked cost begin
(unbanked_cost_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a2, w5_unbanked))), weighted.mean(as003_a2, w5_unbanked)))
#
table(i5_unbanked.df$as003_b2, exclude = F)
(unbanked_cost_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b2, w5_unbanked))), weighted.mean(as003_b2, w5_unbanked)))
#
table(i5_unbanked.df$as003_c2, exclude = F)
(unbanked_cost_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c2, w5_unbanked))), weighted.mean(as003_c2, w5_unbanked)))
#
table(i5_unbanked.df$as003_d2, exclude = F)
(unbanked_cost_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d2, w5_unbanked))), weighted.mean(as003_d2, w5_unbanked)))
#
table(i5_unbanked.df$as003_e2, exclude = F)
(unbanked_cost_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e2, w5_unbanked))), weighted.mean(as003_e2, w5_unbanked)))
#
table(i5_unbanked.df$as003_h2, exclude = F)
(unbanked_cost_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h2, w5_unbanked))), weighted.mean(as003_h2, w5_unbanked)))
#
table(i5_unbanked.df$as003_i2, exclude = F)
(unbanked_cost_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i2, w5_unbanked))), weighted.mean(as003_i2, w5_unbanked)))
#
# Cost assessment data frame
(assess_cost_var.vec = c("Cost_of_cash", "Cost_of_check", "Cost_of_debit", "Cost_of_credit", "Cost_of_prepaid", "Cost_of_moneyorder", "Cost_of_mobile"))
#
(assess_cost_banked.vec = c(banked_cost_cash, banked_cost_check, banked_cost_debit, banked_cost_credit, banked_cost_prepaid, banked_cost_money_order, banked_cost_mobile))
#
(assess_cost_unbanked.vec = c(unbanked_cost_cash, unbanked_cost_check, unbanked_cost_debit, unbanked_cost_credit, unbanked_cost_prepaid, unbanked_cost_money_order, unbanked_cost_mobile))
#
(assess_cost.df = data.frame(Payment_instrument = assess_cost_var.vec, Banked = assess_cost_banked.vec, Unbanked = assess_cost_unbanked.vec))
# export to CSV
write.csv(assess_cost.df, "assess_cost.csv", row.names = F)

# banked security begin
(banked_security_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a3, w5_banked))), weighted.mean(as003_a3, w5_banked)))
#
table(i5_banked.df$as003_b3, exclude = F)
(banked_security_check = with(na.omit(subset(i5_banked.df, select = c(as003_b3, w5_banked))), weighted.mean(as003_b3, w5_banked)))
#
table(i5_banked.df$as003_c3, exclude = F)
(banked_security_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c3, w5_banked))), weighted.mean(as003_c3, w5_banked)))
#
table(i5_banked.df$as003_d3, exclude = F)
(banked_convenience_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d3, w5_banked))), weighted.mean(as003_d3, w5_banked)))
#
table(i5_banked.df$as003_e3, exclude = F)
(banked_convenience_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e3, w5_banked))), weighted.mean(as003_e3, w5_banked)))
#
table(i5_banked.df$as003_h3, exclude = F)
(banked_convenience_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h3, w5_banked))), weighted.mean(as003_h3, w5_banked)))
#
table(i5_banked.df$as003_i3, exclude = F)
(banked_convenience_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i3, w5_banked))), weighted.mean(as003_i3, w5_banked)))
#
# UNbanked convenience begin
(unbanked_convenience_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a3, w5_unbanked))), weighted.mean(as003_a3, w5_unbanked)))
#
table(i5_unbanked.df$as003_b3, exclude = F)
(unbanked_convenience_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b3, w5_unbanked))), weighted.mean(as003_b3, w5_unbanked)))
#
table(i5_unbanked.df$as003_c3, exclude = F)
(unbanked_convenience_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c3, w5_unbanked))), weighted.mean(as003_c3, w5_unbanked)))
#
table(i5_unbanked.df$as003_d3, exclude = F)
(unbanked_convenience_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d3, w5_unbanked))), weighted.mean(as003_d3, w5_unbanked)))
#
table(i5_unbanked.df$as003_e3, exclude = F)
(unbanked_convenience_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e3, w5_unbanked))), weighted.mean(as003_e3, w5_unbanked)))
#
table(i5_unbanked.df$as003_h3, exclude = F)
(unbanked_convenience_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h3, w5_unbanked))), weighted.mean(as003_h3, w5_unbanked)))
#
table(i5_unbanked.df$as003_i3, exclude = F)
(unbanked_convenience_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i3, w5_unbanked))), weighted.mean(as003_i3, w5_unbanked)))

# convenience assessment data frame
(assess_convenience_var.vec = c("Convenience_of_cash", "Convenience_of_check", "Convenience_of_debit", "Convenience_of_credit", "Convenience_of_prepaid", "Convenience_of_moneyorder", "Convenience_of_mobile"))
#
(assess_convenience_banked.vec = c(banked_convenience_cash, banked_convenience_check, banked_convenience_debit, banked_convenience_credit, banked_convenience_prepaid, banked_convenience_money_order, banked_convenience_mobile))
#
(assess_convenience_unbanked.vec = c(unbanked_convenience_cash, unbanked_convenience_check, unbanked_convenience_debit, unbanked_convenience_credit, unbanked_convenience_prepaid, unbanked_convenience_money_order, unbanked_convenience_mobile))
#
(assess_convenience.df = data.frame(Payment_instrument = assess_convenience_var.vec, Banked = assess_convenience_banked.vec, Unbanked = assess_convenience_unbanked.vec))
#
write.csv(assess_convenience.df, "assess_convenience.csv", row.names = F)

## security assessment table
table(i5_banked.df$as003_a4, exclude = F)
str(i5.df$as003_a4)
# banked cost begin
(banked_security_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a4, w5_banked))), weighted.mean(as003_a4, w5_banked)))
#
table(i5_banked.df$as003_b4, exclude = F)
(banked_security_check = with(na.omit(subset(i5_banked.df, select = c(as003_b4, w5_banked))), weighted.mean(as003_b4, w5_banked)))
#
table(i5_banked.df$as003_c4, exclude = F)
(banked_security_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c4, w5_banked))), weighted.mean(as003_c4, w5_banked)))
#
table(i5_banked.df$as003_d4, exclude = F)
(banked_security_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d4, w5_banked))), weighted.mean(as003_d4, w5_banked)))
#
table(i5_banked.df$as003_e4, exclude = F)
(banked_security_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e4, w5_banked))), weighted.mean(as003_e4, w5_banked)))
#
table(i5_banked.df$as003_h4, exclude = F)
(banked_security_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h4, w5_banked))), weighted.mean(as003_h4, w5_banked)))
#
table(i5_banked.df$as003_i4, exclude = F)
(banked_security_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i4, w5_banked))), weighted.mean(as003_i4, w5_banked)))
#
# unbanked security begin
table(i5_unbanked.df$as003_a4, exclude = F)
(unbanked_security_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a4, w5_unbanked))), weighted.mean(as003_a4, w5_unbanked)))
#
table(i5_unbanked.df$as003_b4, exclude = F)
(unbanked_security_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b4, w5_unbanked))), weighted.mean(as003_b4, w5_unbanked)))
#
table(i5_unbanked.df$as003_c4, exclude = F)
(unbanked_security_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c4, w5_unbanked))), weighted.mean(as003_c4, w5_unbanked)))
#
table(i5_unbanked.df$as003_d4, exclude = F)
(unbanked_security_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d4, w5_unbanked))), weighted.mean(as003_d4, w5_unbanked)))
#
table(i5_unbanked.df$as003_e4, exclude = F)
(unbanked_security_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e4, w5_unbanked))), weighted.mean(as003_e4, w5_unbanked)))
#
table(i5_unbanked.df$as003_h4, exclude = F)
(unbanked_security_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h4, w5_unbanked))), weighted.mean(as003_h4, w5_unbanked)))
#
table(i5_unbanked.df$as003_i4, exclude = F)
(unbanked_security_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i4, w5_unbanked))), weighted.mean(as003_i4, w5_unbanked)))

# security assessment data frame
(assess_security_var.vec = c("security_of_cash", "security_of_check", "security_of_debit", "security_of_credit", "security_of_prepaid", "security_of_moneyorder", "security_of_mobile"))
#
(assess_security_banked.vec = c(banked_security_cash, banked_security_check, banked_security_debit, banked_security_credit, banked_security_prepaid, banked_security_money_order, banked_security_mobile))
#
(assess_security_unbanked.vec = c(unbanked_security_cash, unbanked_security_check, unbanked_security_debit, unbanked_security_credit, unbanked_security_prepaid, unbanked_security_money_order, unbanked_security_mobile))
#
(assess_security.df = data.frame(Payment_instrument = assess_security_var.vec, Banked = assess_security_banked.vec, Unbanked = assess_security_unbanked.vec))
#
write.csv(assess_security.df, "assess_security.csv", row.names = F)

## setup assessment table begins
# banked setup begin
table(i5_banked.df$as003_a5, exclude = F)
str(i5.df$as003_a5)
(banked_setup_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a5, w5_banked))), weighted.mean(as003_a5, w5_banked)))
#
table(i5_banked.df$as003_b5, exclude = F)
(banked_setup_check = with(na.omit(subset(i5_banked.df, select = c(as003_b5, w5_banked))), weighted.mean(as003_b5, w5_banked)))
#
table(i5_banked.df$as003_c5, exclude = F)
(banked_setup_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c5, w5_banked))), weighted.mean(as003_c5, w5_banked)))
#
table(i5_banked.df$as003_d5, exclude = F)
(banked_setup_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d5, w5_banked))), weighted.mean(as003_d5, w5_banked)))
#
table(i5_banked.df$as003_e5, exclude = F)
(banked_setup_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e5, w5_banked))), weighted.mean(as003_e5, w5_banked)))
#
table(i5_banked.df$as003_h5, exclude = F)
(banked_setup_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h5, w5_banked))), weighted.mean(as003_h5, w5_banked)))
#
table(i5_banked.df$as003_i5, exclude = F)
(banked_setup_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i5, w5_banked))), weighted.mean(as003_i5, w5_banked)))
# UNbanked setup begin
(unbanked_setup_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a5, w5_unbanked))), weighted.mean(as003_a5, w5_unbanked)))
#
table(i5_unbanked.df$as003_b5, exclude = F)
(unbanked_setup_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b5, w5_unbanked))), weighted.mean(as003_b5, w5_unbanked)))
#
table(i5_unbanked.df$as003_c5, exclude = F)
(unbanked_setup_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c5, w5_unbanked))), weighted.mean(as003_c5, w5_unbanked)))
#
table(i5_unbanked.df$as003_d5, exclude = F)
(unbanked_setup_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d5, w5_unbanked))), weighted.mean(as003_d5, w5_unbanked)))
#
table(i5_unbanked.df$as003_e5, exclude = F)
(unbanked_setup_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e5, w5_unbanked))), weighted.mean(as003_e5, w5_unbanked)))
#
table(i5_unbanked.df$as003_h5, exclude = F)
(unbanked_setup_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h5, w5_unbanked))), weighted.mean(as003_h5, w5_unbanked)))
#
table(i5_unbanked.df$as003_i5, exclude = F)
(unbanked_setup_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i5, w5_unbanked))), weighted.mean(as003_i5, w5_unbanked)))

# setup assessment data frame
(assess_setup_var.vec = c("setup_of_cash", "setup_of_check", "setup_of_debit", "setup_of_credit", "setup_of_prepaid", "setup_of_moneyorder", "setup_of_mobile"))
#
(assess_setup_banked.vec = c(banked_setup_cash, banked_setup_check, banked_setup_debit, banked_setup_credit, banked_setup_prepaid, banked_setup_money_order, banked_setup_mobile))
#
(assess_setup_unbanked.vec = c(unbanked_setup_cash, unbanked_setup_check, unbanked_setup_debit, unbanked_setup_credit, unbanked_setup_prepaid, unbanked_setup_money_order, unbanked_setup_mobile))
#
(assess_setup.df = data.frame(Payment_instrument = assess_setup_var.vec, Banked = assess_setup_banked.vec, Unbanked = assess_setup_unbanked.vec))
#
write.csv(assess_setup.df, "assess_setup.csv", row.names = F)

## acceptance assessment table begins
# banked acceptance begin
table(i5_banked.df$as003_a1, exclude = F)
str(i5.df$as003_a1)
(banked_acceptance_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a1, w5_banked))), weighted.mean(as003_a1, w5_banked)))
#
table(i5_banked.df$as003_b1, exclude = F)
(banked_acceptance_check = with(na.omit(subset(i5_banked.df, select = c(as003_b1, w5_banked))), weighted.mean(as003_b1, w5_banked)))
#
table(i5_banked.df$as003_c1, exclude = F)
(banked_acceptance_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c1, w5_banked))), weighted.mean(as003_c1, w5_banked)))
#
table(i5_banked.df$as003_d1, exclude = F)
(banked_acceptance_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d1, w5_banked))), weighted.mean(as003_d1, w5_banked)))
#
table(i5_banked.df$as003_e1, exclude = F)
(banked_acceptance_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e1, w5_banked))), weighted.mean(as003_e1, w5_banked)))
#
table(i5_banked.df$as003_h1, exclude = F)
(banked_acceptance_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h1, w5_banked))), weighted.mean(as003_h1, w5_banked)))
#
table(i5_banked.df$as003_i1, exclude = F)
(banked_acceptance_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i1, w5_banked))), weighted.mean(as003_i1, w5_banked)))
#
# UNbanked acceptance begin
table(i5_unbanked.df$as003_a1, exclude = F)
str(i5.df$as003_a1)
(unbanked_acceptance_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a1, w5_unbanked))), weighted.mean(as003_a1, w5_unbanked)))
#
table(i5_unbanked.df$as003_b1, exclude = F)
(unbanked_acceptance_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b1, w5_unbanked))), weighted.mean(as003_b1, w5_unbanked)))
#
table(i5_unbanked.df$as003_c1, exclude = F)
(unbanked_acceptance_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c1, w5_unbanked))), weighted.mean(as003_c1, w5_unbanked)))
#
table(i5_unbanked.df$as003_d1, exclude = F)
(unbanked_acceptance_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d1, w5_unbanked))), weighted.mean(as003_d1, w5_unbanked)))
#
table(i5_unbanked.df$as003_e1, exclude = F)
(unbanked_acceptance_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e1, w5_unbanked))), weighted.mean(as003_e1, w5_unbanked)))
#
table(i5_unbanked.df$as003_h1, exclude = F)
(unbanked_acceptance_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h1, w5_unbanked))), weighted.mean(as003_h1, w5_unbanked)))
#
table(i5_unbanked.df$as003_i1, exclude = F)
(unbanked_acceptance_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i1, w5_unbanked))), weighted.mean(as003_i1, w5_unbanked)))

# acceptance assessment data frame
(assess_acceptance_var.vec = c("acceptance_of_cash", "acceptance_of_check", "acceptance_of_debit", "acceptance_of_credit", "acceptance_of_prepaid", "acceptance_of_moneyorder", "acceptance_of_mobile"))
#
(assess_acceptance_banked.vec = c(banked_acceptance_cash, banked_acceptance_check, banked_acceptance_debit, banked_acceptance_credit, banked_acceptance_prepaid, banked_acceptance_money_order, banked_acceptance_mobile))
#
(assess_acceptance_unbanked.vec = c(unbanked_acceptance_cash, unbanked_acceptance_check, unbanked_acceptance_debit, unbanked_acceptance_credit, unbanked_acceptance_prepaid, unbanked_acceptance_money_order, unbanked_acceptance_mobile))
#
(assess_acceptance.df = data.frame(Payment_instrument = assess_acceptance_var.vec, Banked = assess_acceptance_banked.vec, Unbanked = assess_acceptance_unbanked.vec))
#
write.csv(assess_acceptance.df, "assess_acceptance.csv", row.names = F)

## record assessment table begins
# banked record begin
table(i5_banked.df$as003_a6, exclude = F)
str(i5.df$as003_a6)
(banked_record_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a6, w5_banked))), weighted.mean(as003_a6, w5_banked)))
#
table(i5_banked.df$as003_b6, exclude = F)
(banked_record_check = with(na.omit(subset(i5_banked.df, select = c(as003_b6, w5_banked))), weighted.mean(as003_b6, w5_banked)))
#
table(i5_banked.df$as003_c6, exclude = F)
(banked_record_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c6, w5_banked))), weighted.mean(as003_c6, w5_banked)))
#
table(i5_banked.df$as003_d6, exclude = F)
(banked_record_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d6, w5_banked))), weighted.mean(as003_d6, w5_banked)))
#
table(i5_banked.df$as003_e6, exclude = F)
(banked_record_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e6, w5_banked))), weighted.mean(as003_e6, w5_banked)))
#
table(i5_banked.df$as003_h6, exclude = F)
(banked_record_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h6, w5_banked))), weighted.mean(as003_h6, w5_banked)))
#
table(i5_banked.df$as003_i6, exclude = F)
(banked_record_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i6, w5_banked))), weighted.mean(as003_i6, w5_banked)))
# unbanked record begin
table(i5_unbanked.df$as003_a6, exclude = F)
str(i5.df$as003_a6)
(unbanked_record_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a6, w5_unbanked))), weighted.mean(as003_a6, w5_unbanked)))
#
table(i5_unbanked.df$as003_b6, exclude = F)
(unbanked_record_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b6, w5_unbanked))), weighted.mean(as003_b6, w5_unbanked)))
#
table(i5_unbanked.df$as003_c6, exclude = F)
(unbanked_record_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c6, w5_unbanked))), weighted.mean(as003_c6, w5_unbanked)))
#
table(i5_unbanked.df$as003_d6, exclude = F)
(unbanked_record_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d6, w5_unbanked))), weighted.mean(as003_d6, w5_unbanked)))
#
table(i5_unbanked.df$as003_e6, exclude = F)
(unbanked_record_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e6, w5_unbanked))), weighted.mean(as003_e6, w5_unbanked)))
#
table(i5_unbanked.df$as003_h6, exclude = F)
(unbanked_record_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h6, w5_unbanked))), weighted.mean(as003_h6, w5_unbanked)))
#
table(i5_unbanked.df$as003_i6, exclude = F)
(unbanked_record_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i6, w5_unbanked))), weighted.mean(as003_i6, w5_unbanked)))

# record assessment data frame
(assess_record_var.vec = c("record_of_cash", "record_of_check", "record_of_debit", "record_of_credit", "record_of_prepaid", "record_of_moneyorder", "record_of_mobile"))
#
(assess_record_banked.vec = c(banked_record_cash, banked_record_check, banked_record_debit, banked_record_credit, banked_record_prepaid, banked_record_money_order, banked_record_mobile))
#
(assess_record_unbanked.vec = c(unbanked_record_cash, unbanked_record_check, unbanked_record_debit, unbanked_record_credit, unbanked_record_prepaid, unbanked_record_money_order, unbanked_record_mobile))
#
(assess_record.df = data.frame(Payment_instrument = assess_record_var.vec, Banked = assess_record_banked.vec, Unbanked = assess_record_unbanked.vec))
#
write.csv(assess_record.df, "assess_record.csv", row.names = F)

## speed assessment table begins
# banked speed begin
table(i5_banked.df$as003_a7, exclude = F)
str(i5.df$as003_a7)
(banked_speed_cash = with(na.omit(subset(i5_banked.df, select = c(as003_a7, w5_banked))), weighted.mean(as003_a7, w5_banked)))
#
table(i5_banked.df$as003_b7, exclude = F)
(banked_speed_check = with(na.omit(subset(i5_banked.df, select = c(as003_b7, w5_banked))), weighted.mean(as003_b7, w5_banked)))
#
table(i5_banked.df$as003_c7, exclude = F)
(banked_speed_debit = with(na.omit(subset(i5_banked.df, select = c(as003_c7, w5_banked))), weighted.mean(as003_c7, w5_banked)))
#
table(i5_banked.df$as003_d7, exclude = F)
(banked_speed_credit = with(na.omit(subset(i5_banked.df, select = c(as003_d7, w5_banked))), weighted.mean(as003_d7, w5_banked)))
#
table(i5_banked.df$as003_e7, exclude = F)
(banked_speed_prepaid = with(na.omit(subset(i5_banked.df, select = c(as003_e7, w5_banked))), weighted.mean(as003_e7, w5_banked)))
#
table(i5_banked.df$as003_h7, exclude = F)
(banked_speed_money_order = with(na.omit(subset(i5_banked.df, select = c(as003_h7, w5_banked))), weighted.mean(as003_h7, w5_banked)))
#
table(i5_banked.df$as003_i7, exclude = F)
(banked_speed_mobile = with(na.omit(subset(i5_banked.df, select = c(as003_i7, w5_banked))), weighted.mean(as003_i7, w5_banked)))
#
# unbanked speed begin
table(i5_unbanked.df$as003_a7, exclude = F)
str(i5.df$as003_a7)
(unbanked_speed_cash = with(na.omit(subset(i5_unbanked.df, select = c(as003_a7, w5_unbanked))), weighted.mean(as003_a7, w5_unbanked)))
#
table(i5_unbanked.df$as003_b7, exclude = F)
(unbanked_speed_check = with(na.omit(subset(i5_unbanked.df, select = c(as003_b7, w5_unbanked))), weighted.mean(as003_b7, w5_unbanked)))
#
table(i5_unbanked.df$as003_c7, exclude = F)
(unbanked_speed_debit = with(na.omit(subset(i5_unbanked.df, select = c(as003_c7, w5_unbanked))), weighted.mean(as003_c7, w5_unbanked)))
#
table(i5_unbanked.df$as003_d7, exclude = F)
(unbanked_speed_credit = with(na.omit(subset(i5_unbanked.df, select = c(as003_d7, w5_unbanked))), weighted.mean(as003_d7, w5_unbanked)))
#
table(i5_unbanked.df$as003_e7, exclude = F)
(unbanked_speed_prepaid = with(na.omit(subset(i5_unbanked.df, select = c(as003_e7, w5_unbanked))), weighted.mean(as003_e7, w5_unbanked)))
#
table(i5_unbanked.df$as003_h7, exclude = F)
(unbanked_speed_money_order = with(na.omit(subset(i5_unbanked.df, select = c(as003_h7, w5_unbanked))), weighted.mean(as003_h7, w5_unbanked)))
#
table(i5_unbanked.df$as003_i7, exclude = F)
(unbanked_speed_mobile = with(na.omit(subset(i5_unbanked.df, select = c(as003_i7, w5_unbanked))), weighted.mean(as003_i7, w5_unbanked)))
#
# speed assessment data frame
(assess_speed_var.vec = c("speed_of_cash", "speed_of_check", "speed_of_debit", "speed_of_credit", "speed_of_prepaid", "speed_of_moneyorder", "speed_of_mobile"))
#
(assess_speed_banked.vec = c(banked_speed_cash, banked_speed_check, banked_speed_debit, banked_speed_credit, banked_speed_prepaid, banked_speed_money_order, banked_speed_mobile))
#
(assess_speed_unbanked.vec = c(unbanked_speed_cash, unbanked_speed_check, unbanked_speed_debit, unbanked_speed_credit, unbanked_speed_prepaid, unbanked_speed_money_order, unbanked_speed_mobile))
#
(assess_speed.df = data.frame(Payment_instrument = assess_speed_var.vec, Banked = assess_speed_banked.vec, Unbanked = assess_speed_unbanked.vec))
#
write.csv(assess_speed.df, "assess_speed.csv", row.names = F)

### Start Figure 7 
# check, debit, credit, prepaid, money order (mon_adopt), BANP, OBBP, crypto, mobile_app (nbop_acnt_adopt), other_mobile_app (other_nbops_adopt)
# check adoption
table(i5_banked.df$chk_adopt, exclude = F)
(adopt_check_banked_w_frac = sum(i5_banked.df[i5_banked.df$chk_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$chk_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$chk_adopt==0, ]$w5_banked, na.rm = T)))
# debit adoption
table(i5_banked.df$dc_adopt, exclude = F)
(adopt_debit_banked_w_frac = sum(i5_banked.df[i5_banked.df$dc_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$dc_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$dc_adopt==0, ]$w5_banked, na.rm = T)))
# credit adoption
table(i5_banked.df$cc_adopt, exclude = F)
(adopt_credit_banked_w_frac = sum(i5_banked.df[i5_banked.df$cc_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$cc_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$cc_adopt==0, ]$w5_banked, na.rm = T)))
# prepaid adoption
table(i5_banked.df$svc_adopt, exclude = F)
(adopt_prepaid_banked_w_frac = sum(i5_banked.df[i5_banked.df$svc_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$svc_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$svc_adopt==0, ]$w5_banked, na.rm = T)))
# BANP adoption
table(i5_banked.df$banp_adopt, exclude = F)
(adopt_banp_banked_w_frac = sum(i5_banked.df[i5_banked.df$banp_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$banp_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$banp_adopt==0, ]$w5_banked, na.rm = T)))
# OBBP adoption
table(i5_banked.df$obbp_adopt, exclude = F)
(adopt_obbp_banked_w_frac = sum(i5_banked.df[i5_banked.df$obbp_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$obbp_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$obbp_adopt==0, ]$w5_banked, na.rm = T)))
# money order adoption
table(i5_banked.df$mon_adopt, exclude = F)
(adopt_moneyorder_banked_w_frac = sum(i5_banked.df[i5_banked.df$mon_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$mon_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$mon_adopt==0, ]$w5_banked, na.rm = T)))
# crypto adoption
table(i5_banked.df$crypto_adopt, exclude = F)
(adopt_crypto_banked_w_frac = sum(i5_banked.df[i5_banked.df$crypto_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$crypto_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$crypto_adopt==0, ]$w5_banked, na.rm = T)))
# mobile adoption
table(i5_banked.df$nbop_acnt_adopt, exclude = F)
(adopt_mobile_banked_w_frac = sum(i5_banked.df[i5_banked.df$nbop_acnt_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$nbop_acnt_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$nbop_acnt_adopt==0, ]$w5_banked, na.rm = T)))
# other mobile adoption
table(i5_banked.df$other_nbops_adopt, exclude = F)
(adopt_other_mobile_banked_w_frac = sum(i5_banked.df[i5_banked.df$other_nbops_adopt==1, ]$w5_banked, na.rm = T)/(sum(i5_banked.df[i5_banked.df$other_nbops_adopt==1, ]$w5_banked, na.rm = T) + sum(i5_banked.df[i5_banked.df$other_nbops_adopt==0, ]$w5_banked, na.rm = T)))
#
# unbanked adoption begins
# check adoption
table(i5_unbanked.df$chk_adopt, exclude = F)
(adopt_check_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$chk_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$chk_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$chk_adopt==0, ]$w5_unbanked, na.rm = T)))
# debit adoption
table(i5_unbanked.df$dc_adopt, exclude = F)
(adopt_debit_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$dc_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$dc_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$dc_adopt==0, ]$w5_unbanked, na.rm = T)))
# credit adoption
table(i5_unbanked.df$cc_adopt, exclude = F)
(adopt_credit_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$cc_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$cc_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$cc_adopt==0, ]$w5_unbanked, na.rm = T)))
# prepaid adoption
table(i5_unbanked.df$svc_adopt, exclude = F)
(adopt_prepaid_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$svc_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$svc_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$svc_adopt==0, ]$w5_unbanked, na.rm = T)))
# BANP adoption => set to 0 (unbanked cannot have BANP)
table(i5_unbanked.df$banp_adopt, exclude = F)
#(adopt_banp_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$banp_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$banp_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$banp_adopt==0, ]$w5_unbanked, na.rm = T)))
(adopt_banp_unbanked_w_frac = 0)
# OBBP adoption => set to zero (unbanked cannot have OBBP)
table(i5_unbanked.df$obbp_adopt, exclude = F)
#(adopt_obbp_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$obbp_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$obbp_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$obbp_adopt==0, ]$w5_unbanked, na.rm = T)))
(adopt_obbp_unbanked_w_frac = 0)
# money order adoption
table(i5_unbanked.df$mon_adopt, exclude = F)
(adopt_moneyorder_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$mon_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$mon_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$mon_adopt==0, ]$w5_unbanked, na.rm = T)))
# crypto adoption
table(i5_unbanked.df$crypto_adopt, exclude = F)
(adopt_crypto_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$crypto_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$crypto_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$crypto_adopt==0, ]$w5_unbanked, na.rm = T)))
# mobile adoption
table(i5_unbanked.df$nbop_acnt_adopt, exclude = F)
(adopt_mobile_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$nbop_acnt_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$nbop_acnt_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$nbop_acnt_adopt==0, ]$w5_unbanked, na.rm = T)))
# other mobile adoption
table(i5_unbanked.df$other_nbops_adopt, exclude = F)
(adopt_other_mobile_unbanked_w_frac = sum(i5_unbanked.df[i5_unbanked.df$other_nbops_adopt==1, ]$w5_unbanked, na.rm = T)/(sum(i5_unbanked.df[i5_unbanked.df$other_nbops_adopt==1, ]$w5_unbanked, na.rm = T) + sum(i5_unbanked.df[i5_unbanked.df$other_nbops_adopt==0, ]$w5_unbanked, na.rm = T)))

# Average and median number of credit cards [not used because the computations below are for those who have at leasts 1 card]
nrow(i5_banked.df)
sum(i5_banked.df$w5_banked)
nrow(i5_unbanked.df)
sum(i5_unbanked.df$w5_unbanked)
#
table(i5_banked.df$cc_num)
table(i5_unbanked.df$cc_num)
#
(adopt_banked_cc_num_avg_w = weighted.mean(i5_banked.df$cc_num, i5_banked.df$w5_banked, na.rm = T))
#
(adopt_banked_cc_num_med_w = weighted.median(i5_banked.df$cc_num, i5_banked.df$w5_banked, na.rm = T))
#
(adopt_unbanked_cc_num_avg_w = weighted.mean(i5_unbanked.df$cc_num, i5_unbanked.df$w5_unbanked, na.rm = T))
#
(adopt_unbanked_cc_num_med_w = weighted.median(i5_unbanked.df$cc_num, i5_unbanked.df$w5_unbanked, na.rm = T))
# end of not used

# adoption data frame
(adopt_var.vec = c("Adopt_check (%)", "Adopt_debit (%)", "Adopt_credit (%)", "Adopt_prepaid (%)", "Adopt_BANP (%)", "Adopt_OBBP (%)", "Adopt_moneyorder (%)", "Adopt_crypto (%)", "Adopt_mobile (%)", "Adopt_other_mobile (%)"))

(banked_adopt.vec = c(adopt_check_banked_w_frac, adopt_debit_banked_w_frac, adopt_credit_banked_w_frac, adopt_prepaid_banked_w_frac, adopt_banp_banked_w_frac, adopt_obbp_banked_w_frac, adopt_moneyorder_banked_w_frac, adopt_crypto_banked_w_frac, adopt_mobile_banked_w_frac, adopt_other_mobile_banked_w_frac))
#
(unbanked_adopt.vec = c(adopt_check_unbanked_w_frac, adopt_debit_unbanked_w_frac, adopt_credit_unbanked_w_frac, adopt_prepaid_unbanked_w_frac, adopt_banp_unbanked_w_frac, adopt_obbp_unbanked_w_frac, adopt_moneyorder_unbanked_w_frac, adopt_crypto_unbanked_w_frac, adopt_mobile_unbanked_w_frac, adopt_other_mobile_unbanked_w_frac))
#
(adopt.df = data.frame(Payment_method = adopt_var.vec, Banked = 100* banked_adopt.vec, Unbanked = 100* unbanked_adopt.vec))
#
write.csv(adopt.df, "figure7.csv", row.names = F)

### Start Table 5 in 2016 paper (cash holding)
# need the day-level data
names(d1.df)
d2.df = data.frame(d1.df)# get rid of tibble
table(d2.df$diary_day)  
summary(d2.df$end_cash_bal)
summary(d2.df$stored_cash_bal)
#
# import id of banked and unbanked into d2.df
names(dplyr::select(i5.df, contains("bank")))
dim(i5.df)
head(i5.df$banked)
head(i5.df$unbanked)

#adding a column "banked" to d2.df
banked.df = subset(i5.df, select = c(id, banked))
dim(banked.df)
banked.df = data.frame(banked.df)
head(banked.df)
#
d3.df = merge(d2.df, banked.df, by = "id", all.y = T)
dim(d3.df)
length(unique(d3.df$id))
length(unique(subset(d3.df, banked==1)$id))# num banked
length(unique(subset(d3.df, banked==0)$id))# num unbanked

# finalizing cash holding stats for Table 5 in 2016 paper
summary(d3.df$end_cash_bal)
summary(d3.df$stored_cash_bal)
#
nrow(d3.df)
sum(d3.df$ind_weight)
# banked
#(cash_stored_banked_avg = weighted.mean(subset(d3.df, banked==1)$stored_cash_bal, subset(d3.df, banked==1)$ind_weight, na.rm = T))
# remove from above resp storing > $10k
(cash_stored_banked_avg = weighted.mean(subset(d3.df, banked==1 & stored_cash_bal <= 10000)$stored_cash_bal, subset(d3.df, banked==1& stored_cash_bal <= 10000)$ind_weight, na.rm = T))
#
(cash_on_person_banked_avg = weighted.mean(subset(d3.df, banked==1 & stored_cash_bal <= 10000)$end_cash_bal, subset(d3.df, banked==1 & stored_cash_bal <= 10000)$ind_weight))
# total holding banked
(cash_holding_banked_avg = weighted.mean(subset(d3.df, banked==1 & stored_cash_bal <= 10000)$stored_cash_bal + subset(d3.df, banked==1 & stored_cash_bal <= 10000)$end_cash_bal, subset(d3.df, banked==1 & stored_cash_bal <= 10000)$ind_weight, na.rm = T))

# UNbanked
(cash_stored_unbanked_avg = weighted.mean(subset(d3.df, banked==0& stored_cash_bal <= 10000)$stored_cash_bal, subset(d3.df, banked==0& stored_cash_bal <= 10000)$ind_weight, na.rm = T))
#
(cash_on_person_unbanked_avg = weighted.mean(subset(d3.df, banked==0 & stored_cash_bal <= 10000)$end_cash_bal, subset(d3.df, banked==0 & stored_cash_bal <= 10000)$ind_weight))
#
# total holding unbanked
(cash_holding_unbanked_avg = weighted.mean(subset(d3.df, banked==0 & stored_cash_bal <= 10000)$stored_cash_bal + subset(d3.df, banked==0 & stored_cash_bal <= 10000)$end_cash_bal, subset(d3.df, banked==0 & stored_cash_bal <= 10000)$ind_weight, na.rm = T))
#
# computing average total holding as percentage of weekly HH income
nrow(i5.df)
length(unique(d3.df$id))
nrow(i5_banked.df) + nrow(i5_unbanked.df)
# Avg HH income: Banked
(hh_income_banked_avg = weighted.mean(i5_banked.df$income_hh, i5_banked.df$ind_weight, na.rm = T))
# Avg HH income: UNbanked
(hh_income_unbanked_avg = weighted.mean(i5_unbanked.df$income_hh, i5_unbanked.df$ind_weight, na.rm = T))

# Avg cash holding as fraction of average HH income
(cash_holding_per_weekly_income_banked = cash_holding_banked_avg/(hh_income_banked_avg/52))
#
(cash_holding_per_weekly_income_unbanked = cash_holding_unbanked_avg/(hh_income_unbanked_avg/52))

# visited check-cashing store (past 12 month)
table(i5_banked.df$used_chkcashing)
table(i5_unbanked.df$used_chkcashing)
#
(used_chkcashing_banked_avg = weighted.mean(i5_banked.df$used_chkcashing, i5_banked.df$ind_weight, na.rm = T))
#
(used_chkcashing_unbanked_avg = weighted.mean(i5_unbanked.df$used_chkcashing, i5_unbanked.df$ind_weight, na.rm = T))

# finalizing Table 5 in 2016 paper
(holding_banked.vec = c(cash_holding_banked_avg, cash_on_person_banked_avg, cash_stored_banked_avg, 100*cash_holding_per_weekly_income_banked, 100*used_chkcashing_banked_avg))
#
(holding_unbanked.vec = c(cash_holding_unbanked_avg, cash_on_person_unbanked_avg, cash_stored_unbanked_avg, 100*cash_holding_per_weekly_income_unbanked, 100*used_chkcashing_unbanked_avg))
#
(holding_var.vec = c("Cash_holding ($)", "Cash_on_person ($)", "Cash_stored ($)", "Cash_holding_per_weekly_HH_income (%)", "Used_check_cashing_store (%)" ))

(holding_cash.df = data.frame(Variable = holding_var.vec, Banked = holding_banked.vec, Unbanked = holding_unbanked.vec))

write.csv(holding_cash.df, "table5.csv", row.names = F)

# regarding Table 5 above, how many banked store more than $10 cash?
length(unique(subset(d3.df, banked==1 & stored_cash_bal > 10000)$id))
# out of banked
length(unique(subset(d3.df, banked==1)$id))
#
# any unbanked storing more than $10k? => No.
length(unique(subset(d3.df, banked==0 & stored_cash_bal > 10000)$id))


### Start Table 6 (SVC) + Figure 8 in 2016 paper
names(dplyr::select(i5.df, contains("adopt")))

# adoption SVC
table(i5_banked.df$svc_adopt, exclude = F)
table(i5_unbanked.df$svc_adopt, exclude = F)
#
(svc_adopt_banked_frac_w = sum(subset(i5_banked.df, svc_adopt==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(svc_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, svc_adopt==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# PP card use
table(i5_banked.df$pa050_svc, exclude = F)
table(i5_unbanked.df$pa050_svc, exclude = F)
#
(svc_use_banked_frac_w = sum(subset(i5_banked.df, pa050_svc==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(svc_use_unbanked_frac_w = sum(subset(i5_unbanked.df, pa050_svc==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#

# EBT etc adopt
table(i5_banked.df$pa198_f, exclude = F)
table(i5_unbanked.df$pa198_f, exclude = F)
#
(ebt_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_f==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(ebt_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_f==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# payroll card adopt
table(i5_banked.df$pa198_g, exclude = F)
table(i5_unbanked.df$pa198_g, exclude = F)
#
(payroll_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_g==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(payroll_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_g==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# Benefit card adopt (FSA, etc)
table(i5_banked.df$pa198_i, exclude = F)
table(i5_unbanked.df$pa198_i, exclude = F)
#
(benefit_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_i==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(benefit_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_i==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# Other prepaid card adopt
table(i5_banked.df$pa198_b, exclude = F)
table(i5_unbanked.df$pa198_b, exclude = F)
#
(other_pp_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_b==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(other_pp_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_b==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# Gift card adopt
table(i5_banked.df$pa198_a)
table(i5_unbanked.df$pa198_a)
#
(gift_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_a==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(gift_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_a==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# Transport card adopt
table(i5_banked.df$pa198_c)
table(i5_unbanked.df$pa198_c)
#
(transp_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_c==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(transp_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_c==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# Rebate card adopt
table(i5_banked.df$pa198_k)
table(i5_unbanked.df$pa198_k)
#
(rebate_adopt_banked_frac_w = sum(subset(i5_banked.df, pa198_k==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(rebate_adopt_unbanked_frac_w = sum(subset(i5_unbanked.df, pa198_k==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# Finalizing Table 6 (prepaid cards) in 2016 paper

(svc_var.vec = c("SVC_adoption (%)", "SVC_use (%)", "Other_prepaid_adoption (%)",  "EBT_WIC_SNAP_TANF_adoption (%)", "Transportation_card_adoption (%)", "FDA_HRA_HSA_healthcare_daycare_adoption (%)", "Payroll_adoption (%)", "Gift_card_adoption (%)", "Rebate_card_adoption (%)"))

(svc_banked.vec = c(svc_adopt_banked_frac_w, svc_use_banked_frac_w, other_pp_adopt_banked_frac_w,  ebt_adopt_banked_frac_w, transp_adopt_banked_frac_w, benefit_adopt_banked_frac_w, payroll_adopt_banked_frac_w, gift_adopt_banked_frac_w, rebate_adopt_banked_frac_w))
#
(svc_unbanked.vec = c(svc_adopt_unbanked_frac_w, svc_use_unbanked_frac_w, other_pp_adopt_unbanked_frac_w,  ebt_adopt_unbanked_frac_w, transp_adopt_unbanked_frac_w, benefit_adopt_unbanked_frac_w, payroll_adopt_unbanked_frac_w, gift_adopt_unbanked_frac_w, rebate_adopt_unbanked_frac_w))

(svc.df = data.frame(Variable = svc_var.vec, Banked = 100*svc_banked.vec, Unbanked = 100* svc_unbanked.vec))
#
write.csv(svc.df, "table6.csv", row.names = F)

### Start Figure 10-11 in 2016 paper [credit score]
adopt_credit_banked_w_frac
adopt_credit_unbanked_w_frac
#
table(i5_banked.df$ph006, exclude = F)
table(i5_unbanked.df$ph006, exclude = F)
# still unweighted
percent(table(i5_banked.df$ph006)/sum(table(i5_banked.df$ph006)))
percent(table(i5_unbanked.df$ph006)/sum(table(i5_unbanked.df$ph006)))
# FICO banked weighted
(fico_banked_1_w = sum(subset(i5_banked.df, ph006==1)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(fico_banked_2_w = sum(subset(i5_banked.df, ph006==2)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(fico_banked_3_w = sum(subset(i5_banked.df, ph006==3)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(fico_banked_4_w = sum(subset(i5_banked.df, ph006==4)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(fico_banked_5_w = sum(subset(i5_banked.df, ph006==5)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(fico_banked_6_w = sum(subset(i5_banked.df, ph006==6)$ind_weight)/sum(i5_banked.df$ind_weight))
#
(fico_banked_7_w = sum(subset(i5_banked.df, ph006==7)$ind_weight)/sum(i5_banked.df$ind_weight))
# FICO UNbanked weighted
(fico_unbanked_1_w = sum(subset(i5_unbanked.df, ph006==1)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#
(fico_unbanked_2_w = sum(subset(i5_unbanked.df, ph006==2)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#
(fico_unbanked_3_w = sum(subset(i5_unbanked.df, ph006==3)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#
(fico_unbanked_4_w = sum(subset(i5_unbanked.df, ph006==4)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#
(fico_unbanked_5_w = sum(subset(i5_unbanked.df, ph006==5)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#
(fico_unbanked_6_w = sum(subset(i5_unbanked.df, ph006==6)$ind_weight)/sum(i5_unbanked.df$ind_weight))
#
(fico_unbanked_7_w = sum(subset(i5_unbanked.df, ph006==7)$ind_weight)/sum(i5_unbanked.df$ind_weight))

# FICO excluding those who don't know their score (Figure 11)
# FICO banked weighted (excluding don't know)
(fico_banked_exclude_1_w = sum(subset(i5_banked.df, ph006==1)$ind_weight)/sum(subset(i5_banked.df, ph006!=7)$ind_weight))
#
(fico_banked_exclude_2_w = sum(subset(i5_banked.df, ph006==2)$ind_weight)/sum(subset(i5_banked.df, ph006!=7)$ind_weight))
#
(fico_banked_exclude_3_w = sum(subset(i5_banked.df, ph006==3)$ind_weight)/sum(subset(i5_banked.df, ph006!=7)$ind_weight))
#
(fico_banked_exclude_4_w = sum(subset(i5_banked.df, ph006==4)$ind_weight)/sum(subset(i5_banked.df, ph006!=7)$ind_weight))
#
(fico_banked_exclude_5_w = sum(subset(i5_banked.df, ph006==5)$ind_weight)/sum(subset(i5_banked.df, ph006!=7)$ind_weight))
#
(fico_banked_exclude_6_w = sum(subset(i5_banked.df, ph006==6)$ind_weight)/sum(subset(i5_banked.df, ph006!=7)$ind_weight))
#
(fico_banked_exclude_7_w = NA)# excluded
# FICO UNbanked weighted (excluding don't know)
(fico_unbanked_exclude_1_w = sum(subset(i5_unbanked.df, ph006==1)$ind_weight)/sum(subset(i5_unbanked.df, ph006!=7)$ind_weight))
#
(fico_unbanked_exclude_2_w = sum(subset(i5_unbanked.df, ph006==2)$ind_weight)/sum(subset(i5_unbanked.df, ph006!=7)$ind_weight))
#
(fico_unbanked_exclude_3_w = sum(subset(i5_unbanked.df, ph006==3)$ind_weight)/sum(subset(i5_unbanked.df, ph006!=7)$ind_weight))
#
(fico_unbanked_exclude_4_w = sum(subset(i5_unbanked.df, ph006==4)$ind_weight)/sum(subset(i5_unbanked.df, ph006!=7)$ind_weight))
#
(fico_unbanked_exclude_5_w = sum(subset(i5_unbanked.df, ph006==5)$ind_weight)/sum(subset(i5_unbanked.df, ph006!=7)$ind_weight))
#
(fico_unbanked_exclude_6_w = sum(subset(i5_unbanked.df, ph006==6)$ind_weight)/sum(subset(i5_unbanked.df, ph006!=7)$ind_weight))
#
(fico_unbanked_exclude_7_w = NA)# excluded

# Finalizing Figures 10-11 in the 2016 paper (FICO score)
(fico_banked.vec = c(fico_banked_1_w, fico_banked_2_w, fico_banked_3_w, fico_banked_4_w, fico_banked_5_w, fico_banked_6_w, fico_banked_7_w))
# verify sums to 1
sum(fico_banked.vec)
#
(fico_unbanked.vec = c(fico_unbanked_1_w, fico_unbanked_2_w, fico_unbanked_3_w, fico_unbanked_4_w, fico_unbanked_5_w, fico_unbanked_6_w, fico_unbanked_7_w))
# verify sums to 1
sum(fico_unbanked.vec)
#
(fico_banked_exclude.vec = c(fico_banked_exclude_1_w, fico_banked_exclude_2_w, fico_banked_exclude_3_w, fico_banked_exclude_4_w, fico_banked_exclude_5_w, fico_banked_exclude_6_w, fico_banked_exclude_7_w))
# verify sums to 1
sum(fico_banked_exclude.vec[1:6])
#
(fico_unbanked_exclude.vec = c(fico_unbanked_exclude_1_w, fico_unbanked_exclude_2_w, fico_unbanked_exclude_3_w, fico_unbanked_exclude_4_w, fico_unbanked_exclude_5_w, fico_unbanked_exclude_6_w, fico_unbanked_exclude_7_w))
# verify sums to 1
sum(fico_unbanked_exclude.vec[1:6])
#
(fico_var.vec = c("Below 600 (%)", "600-649 (%)", "650-699 (%)", "700-749 (%)", "750-800 (%)", "Above 800 (%)", "I don't know (%)"))

# make FICO Figure 10-11 data frame
(fico.df = data.frame(Credit_score = fico_var.vec, Banked = 100*fico_banked.vec,  unbanked = 100*fico_unbanked.vec, Banked_exl_7 = 100*fico_banked_exclude.vec, Unbanked_exl_7 = 100*fico_unbanked_exclude.vec))
#
write.csv(fico.df, "figure_10_11.csv", row.names = F)

### Start hub_fig_2, Figure 2 from the Policy Hub paper, hub_fig_2
...
### Start Hub figure 3 in Policy Hub paper
# Fraction of cash payments in 8 PI payments (banked vs unbanked)
...
### Start Hub-fig 4 in Policy Hub paper (bills by number)
...
# finalizing hub-fig 4
(pi_bill_banked_frac.vec = c(cash_bill_banked_frac_w, check_bill_banked_frac_w, credit_bill_banked_frac_w, debit_bill_banked_frac_w, prepaid_bill_banked_frac_w, banp_bill_banked_frac_w, obbp_bill_banked_frac_w, money_order_bill_banked_frac_w))
#
(pi_bill_unbanked_frac.vec = c(cash_bill_unbanked_frac_w, check_bill_unbanked_frac_w, credit_bill_unbanked_frac_w, debit_bill_unbanked_frac_w, prepaid_bill_unbanked_frac_w, banp_bill_unbanked_frac_w, obbp_bill_unbanked_frac_w, money_order_bill_unbanked_frac_w))
#
...
(pi_bill.df = data.frame(Payment_method = pi_bill_var.vec, Banked = 100*pi_bill_banked_frac.vec, Unbanked = 100* pi_bill_unbanked_frac.vec))
# check sum up to 1
sum(pi_bill.df$Banked)
sum(pi_bill.df$Unbanked)
...
nrow(n3.df)
n3.df = n3.df
n3.df$ind_weight = nrow(n3.df)* m3.df$ind_weight/sum(n3.df$ind_weight)
sum(n3.df$ind_weight)
nrow(n3.df)
...
(num_payments_var.vec = c("Avg_monthly_num_payments", "Avg_monthly_num_bill_payments", "Avg_monthly_num_p2p_payments", "Median_HH_income", "Median_HH_size", "Median_age", "Number_of_respondents"))
#
(num_payments_banked.vec = c(banked_avg_monthly_payments, banked_avg_monthly_bill_payments, banked_avg_monthly_p2p_payments, banked_med_income, banked_med_size, banked_med_age, banked_num_resp))
#
(num_payments_unbanked.vec = c(unbanked_avg_monthly_payments, unbanked_avg_monthly_bill_payments, unbanked_avg_monthly_p2p_payments, unbanked_med_income, unbanked_med_size, unbanked_med_age, unbanked_num_resp))
