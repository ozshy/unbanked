...
# Reference to tables and figures corresponds to those of Cole and Greene BOS Fed 2016
#
...
levels(i6.df$edu_factor) = c("Education_college", "Education_less_than_high_school", "Education_high_school",  "Education_some_college_or_associate", "Education_graduate")
...
unbanked_ame.reg = logitmfx(unbanked ~ age_factor + gender_factor + race_factor + ethnicity_factor +  edu_factor + marital_factor + income_factor + work_factor + hh_size + homeowner_factor , data = i6.df, atmean = F)
# Marginal effects at mean yields less significant variables
#unbanked_mem.reg = logitmfx(unbanked ~ age_factor + gender_factor + race_factor +  ethnicity_factor +  edu_factor + marital_factor + income_factor + work_factor + hh_size + homeowner_factor , data = i6.df, atmean = T)

unbanked_ame_no_income.reg = logitmfx(unbanked ~ age_factor + gender_factor + race_factor +  ethnicity_factor + edu_factor + marital_factor + work_factor + hh_size + homeowner_factor , data = i6.df, atmean = F)

# Saving Table B.1 in 2016 paper
# converting to HTML (can be read by Excel)
htmlreg(list(unbanked_ame.reg, unbanked_ame_no_income.reg), custom.model.names = c("Logit", "Logit (w/o income)"), digits = 4)
...
#
(pi_bill_unbanked_frac.vec = c(cash_unbanked_frac_w, check_unbanked_frac_w, credit_unbanked_frac_w, debit_unbanked_frac_w, prepaid_unbanked_frac_w, banp_unbanked_frac_w, obbp_unbanked_frac_w, money_order_unbanked_frac_w))
#
